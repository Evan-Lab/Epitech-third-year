...
interface HomePageAreaListProps {
  searchKeyword: string;
}

const HomePageAreaList: React.FC<HomePageAreaListProps> = ({ searchKeyword }) => {
  const [query, setQuery] = useState<string | null>(null);
  const [showList, setShowList] = useState(false);

  const toggleShowList = (text: string) => {
    setQuery(text);
    setShowList(!!text);
  };
  ...
  return (
    <ScrollView>
      <Searchbar
        ...
        onChangeText={toggleShowList}
        value={query ?? ''}
      />
      {showList && filteredCards.map((cardTitle) => (
...
export default HomePageAreaList;
