<template>
  <div class="background">
    ...
    <div class="body">
      ...
      <span class="text">{{ $t('Choose your youtube video and youtube plailyst in the trigger') }}</span>
      <input type="text" v-model="urlVideo" :placeholder="$t('Please enter the URL of the youtube video')" class="text-input">
      <input type="text" v-model="urlPlaylist" :placeholder="$t('Please enter the URL of the youtube playlist')" class="text-input">
...
const validateButton = () => {
  if (urlVideo.value === '')
    alert('Please enter a valide Youtube video url');
  else if (urlPlaylist.value === '')
    alert('Please enter a valide Youtube playlist url');
    else {
      const urlObject = { urlVideo: urlVideo.value, urlPlaylist: urlPlaylist.value };
      const urlString = JSON.stringify(urlObject);
      emit('return-value', { value: `${urlString}`, name: 'Youtube(Reaction: VideoToPlaylist)', id:6});
      emit('back-to-areas');
    }
};
