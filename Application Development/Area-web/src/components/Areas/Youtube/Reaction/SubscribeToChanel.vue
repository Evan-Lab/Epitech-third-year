<template>
  <div class="background">
    ...
    <div class="body">
      <img alt="Outlook-logo" class="image" src="@/assets/Youtube.png" width="120" height="110" />
      <span class="text">{{ $t('Subscribe to a youtube channel') }}</span>
      <input type="text" v-model="url" :placeholder="$t('Please enter the URL of the youtube channel')" class="text-input">
...
const validateButton = () => {
  if (url.value === '')
    alert('Please enter a valide Youtube chanel url');
  else {
    const urlObject = { url: url.value };
    const urlString = JSON.stringify(urlObject);
    emit('return-value', { value: `${urlString}`, name: 'Youtube(Reaction: SubscribeToChanel)', id:5});
    emit('back-to-areas');
  }
};
