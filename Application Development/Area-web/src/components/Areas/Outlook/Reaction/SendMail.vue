<template>
  <div class="background">
    <div class="header">
      ...
      <span class="text">{{ $t('Complete empty field') }}</span>
    </div>
    <div class="body">
      <img alt="Outlook-logo" class="image" src="@/assets/Outlook.png" width="300" height="175" />
      <span class="text">{{ $t('Please enter the email adress you want to use') }}</span>
      <input type="email" v-model="mail" :placeholder="$t('Email adress')" class="text-input">
      <button @click="validateEmail" class="validate-button">{{ $t('Create') }}</button>
    </div>
  </div>
</template>
...
const validateEmail = () => {
  if (isEmailValid()) {
    ...
    emit('return-value', { value: `${emailString}`, name: 'Outlook(Reaction: SendMail)',id:1});
    emit('back-to-areas');
  } else {
