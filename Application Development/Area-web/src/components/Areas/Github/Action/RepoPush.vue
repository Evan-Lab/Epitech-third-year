<template>
  <div class="background">
    ...
    <div class="body">
      <img alt="Outlook-logo" class="image" src="@/assets/Github.png" width="120" height="120" />
      <span class="text">{{ $t('Create an action when you push in repository') }}</span>
      <input type="text" v-model="repo" :placeholder="$t('Please enter the repository you want to monitor')" class="text-input">
      <button @click="validateButton" class="validate-button">Create</button>
    </div>
  </div>
</template>
...
const validateButton = () => {
  if (repo.value === '')
    ...
    emit('return-value', { value: `${repoString}`, name: 'Github(Action: RepoPush)', id: 8});
    emit('back-to-areas');
  }
};
