<template>
  <div class="background">
    ...
    <div class="body">
      <img alt="Outlook-logo" class="image" src="@/assets/Spotify.png" width="150" height="150" />
      <span class="text">{{ $t('create an action when you put a music in a playlist') }}</span>
      <input type="text" v-model="url" :placeholder="$t('Please enter the url of the playlist you want to monitor')" class="text-input">
...
const validateButton = () => {
  if (url.value !== '') {
    const idObject = { url: url.value };
    const idString = JSON.stringify(idObject);
    emit('return-value', { value: `${idString}`, name: 'Spotify(Action: NewTrack)',id: 11});
    emit('back-to-areas');
  } else
    alert('Spotify ID invalide. Veuillez saisir une ID de Spotify valide.');
};
